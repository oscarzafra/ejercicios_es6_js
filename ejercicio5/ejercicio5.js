...
function filterStreamers() {
    const input = document.querySelector('[data-function="toFilterStreamers"]').value; 
    const filteredStreamers = streamers.filter(streamer => 
      streamer.name.toLowerCase().includes(input.toLowerCase()) 
    );
    
    console.log(filteredStreamers); 
  }
  
  document.querySelector('[data-function="toFilterStreamers"]').addEventListener('input', filterStreamers);